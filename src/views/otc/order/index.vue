  ...
  methods: {
    ...
    async but_click () {
      if (this.but) {
        ...
        // 买入时做的事情
        if (sessionStorage.getItem('page') === '0') {
          let { data } = await this.$api.otc.orderCreate({ otc_pair: Obj.otc_pair, side: Obj.side, currency_amount: Obj.amount, price: Obj.price, payment_method: payment_method })
          ...
          let { data } = await this.$api.otc.orderCreate({ otc_pair: Obj.otc_pair, side: Obj.side, asset_amount: Obj.amount, price: Obj.price, payment_method: payment_method })
          const id = data.data.id
          // 卖出时做的事情
          if (this.index === 0) {
            // 银行收款的时候做的事情
